<template>
    <!-- <div v-if="!notice.length" class="popup">
        <div class="popup-wrap">
            <div class="popup-header">
                <svg @click="close(item.id)" xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 28 28" fill="none">
    ...
    <div v-if="notice.length" class="popup">
        <div class="popup-wrap" v-for="item in notice" :key="item">
            <div class="popup-header">
                <svg @click="close(item.id)" xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 28 28" fill="none">
                <mask id="mask0_1032_13894" style="mask-type:alpha" maskUnits="userSpaceOnUse" x="0" y="0" width="28" height="28">
                <rect width="28" height="28" fill="#D9D9D9"/>
            ...
            <div class="popup-footer">
                오늘 다시 보지 않기<input type="checkbox" @change="check_not_today_notice($event, item.id)">
            </div>
        </div>
    </div>
</template>
...
const close = (id) => {
    localStorage.setItem(`noticearr`, JSON.stringify(not_today.value))
    notice.value = notice.value.filter(noti => noti.id != id)
}

const notice: Ref<any> = ref([])
// const notice = ref([
//   {
//     title: 1231231231,
...
//   }
// ])
// localStorage.setItem(`noticearr`, '[]')

const not_today:Ref<any> = ref([])

const reset_not_today = () => {
    
    if (localStorage.getItem('noticetoggle') != new Date().toISOString().slice(0,10)) {
        console.log('reset_not_today');

        localStorage.setItem('noticetoggle', new Date().toISOString().slice(0,10))
        localStorage.setItem(`noticearr`, '[]')    
    }
    if (localStorage.getItem('noticetoggle') == new Date().toISOString().slice(0,10)) {
        console.log('localStorage.getItem(`noticearr`)');
        console.log(localStorage.getItem(`noticearr`));
        
        if (localStorage.getItem(`noticearr`)) not_today.value = JSON.parse(localStorage.getItem(`noticearr`))
    }
}

const check_not_today_notice = (e, id) => {
    console.log(e.target.checked);

    if (e.target.checked) {
        not_today.value.push(id)
    }
    else{
        not_today.value = not_today.value.filter(notiId => notiId != id)
    }
}
...
const init = async () => {
  ...
  notice.value = data.filter(noti => !not_today.value.includes(noti.id))
