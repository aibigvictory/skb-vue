<template>
        ...
        <div class="table-body" v-if="favorite_data.length">
          <ul v-for="ul in favorite_data" :key="ul">
            ...
            <li>
              ...
              <div><span class="company">{{company_name(store.getters.getData('company'), ul.file.user.companyId)}}</span></div>
            </li>
            <li>{{ul.file.createdAt ?ul.file.createdAt.slice(0,10) :''}} {{ul.file.createdAt ?ul.file.createdAt.slice(11,19) :''}}</li>
            <li>
              <div>{{ul.file.history[0].user ?ul.file.history[0].user.name : ''}}
                <svg @click="change_popup_state('open', true, ul.file.history)" xmlns="http://www.w3.org/2000/svg" width="16" height="17" viewBox="0 0 16 17" fill="none">
            ...
            <li>
              <span class="adjust" @click="file_click(ul.fileId, ul.file.name, ul.file.user.name, ul.file.user.teamName, ul.file.createdAt, group_name(store.getters.getData('folder'), ul.file.folderCd))">
  ...
  <DashboardFavoritesPopup v-if="popup_state" @exit="change_popup_state('delete', false)" :history="history"/>
</template>
