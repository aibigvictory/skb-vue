<template>
  <div class="dashboard-revision">
    <div class="header">
      ...
      <router-link to="/revi" class="more">
        더보기
        <svg
    ...
    <div class="content">
      ...
      <div class="table-body">
        ...
        <ul v-for="ul in revision_data" :key="ul">
          ...
          <li>
            {{
              `${ul.fileName}${ul.file.version ? `_${ul.file.version}` : ""}.${
                ul.file.extension
              }`
            }}
          </li>
          ...
          <li>
            {{ ul.createdAt ? ul.createdAt.slice(0, 10) : "" }}
            {{ ul.createdAt ? ul.createdAt.slice(11, 19) : "" }}
          </li>
          <li>
            <div>
              {{ ul.user ? ul.user.name : "" }}
              <svg
                @click="change_popup_state('open', true, ul.fileName)"
                xmlns="http://www.w3.org/2000/svg"
                width="16"
                height="17"
                viewBox="0 0 16 17"
            ...
            <div>
              <span class="company">{{
                ul.user && ul.user.department ? ul.user.department : "SKB"
              }}</span>
            </div>
          </li>
          <li>
            {{ ul.updatedAt ? ul.updatedAt.slice(0, 10) : "" }}
            {{ ul.updatedAt ? ul.updatedAt.slice(11, 19) : "" }}
          </li>
          <li>
