  ...
  <div class="wrap">
    <div class="header">
      ...
      <div class="right">
        ...
        <router-link :to="`/header/adjust/${excelId}`" class="btn btn-download"
          >헤더 지정</router-link
        >
      </div>
...
const excelUploaderCompanyName = computed(() => {
  if (
    excelUploaderCompanyId.value == "" ||
    !store.getters.getData("company").length
  )
    return "";
  else {
    return (
      store.getters
        .getData("company")
        .find((con) => con.id == excelUploaderCompanyId.value)?.name ?? ""
    );
  }
});
...
const init = async (id) => {
  ...
  const { name, user, folder, bookmark, revision, version, extension } = data;
