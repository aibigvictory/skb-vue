<template>
  <div class="warp">
    <div class="search-result-header d-flex justify-content-between">
      <div class="title">통합검색 제외</div>
      <button class="load-btn" @click="save_serach_except">설정 저장</button>
    </div>
    <div class="search-result-section d-flex" v-if="data_count">
      <div class="category">
        ...
        <ul class="category-list nav">
          <li v-for="(folder, idx) in folder_list" :key="folder" @click="click_category" :class="{ active: !idx }" data-bs-toggle="tab" :href="`#kt_tab_pane_${idx + 1}${idx + 1}${idx + 1}`">{{folder.name}} <span>{{checked_file_list.filter(con => con.folder.name == folder.name).length}}</span></li>
        </ul>
      </div>
    ...
    <div class="search-result-section d-flex" v-if="!data_count">
      <div class="no-result">
        ...
        <div class="bottom">
          <div class="txt">관리 파일이 없습니다.</div>
        </div>
      </div>
    </div>
    <div class="search-result-section2">
      <div class="title">통합검색 제외 리스트 <span :class="{zero: !checked_file_sum}">{{checked_file_sum}}건</span></div>
      <ul class="search-result-section2-wrap">
        <li v-if="!checked_file_sum">제외한 파일이 없습니다.</li>
...
const init = async () => {
  const { data } = await axios.post(`/file/list`, {type: 'manage'})
  
  const folders = store.getters.getData('folder_manage')
  const files = data
...
.warp{
  .search-result-header{
    ...
    .load-btn{
      ...
      gap: 8px;
      border-radius: 6px;
      background: var(--default-red, #009ef7);
