<template>
  <div class="warp">
    <div class="search-result-header d-flex justify-content-between">
      <div class="title">
        검색결과 <span>"{{ state.search.keyword }}"</span>
      </div>
      ...
      <div class="category">
        <div class="category-title">
          ...
          >
            ...
            >
              <path
                d="M1703 4311 c-12 -6 -204 -131 -427 -280 -223 -149 -408 -271 -411
          ...
          22 81 0 120 -133 198 -235 137z"
              />
              <path
                d="M2495 2706 c-67 -29 -105 -106 -91 -181 9 -47 59 -102 104 -115 25
          -8 401 -10 1194 -8 1150 3 1157 3 1184 24 53 39 69 71 69 134 0 63 -16 95 -69
          134 -27 21 -33 21 -1194 23 -955 2 -1173 0 -1197 -11z"
              />
              <path
                d="M1685 1737 c-22 -13 -210 -137 -418 -276 -208 -139 -385 -255 -392
          -258 -10 -3 -37 41 -89 144 -84 168 -100 185 -186 191 -45 3 -61 0 -89 -20
          -48 -33 -70 -66 -77 -118 -6 -43 0 -58 122 -300 119 -235 132 -257 169 -278
          31 -17 51 -22 88 -19 44 4 92 34 555 343 358 240 514 349 530 373 15 23 22 49
          22 81 0 120 -133 198 -235 137z"
              />
              <path
                d="M2495 1266 c-67 -29 -105 -106 -91 -181 9 -47 59 -102 104 -115 25
          -8 401 -10 1194 -8 1150 3 1157 3 1184 24 53 39 69 71 69 134 0 63 -16 95 -69
          134 -27 21 -33 21 -1194 23 -955 2 -1173 0 -1197 -11z"
              />
            </g>
        ...
        <ul class="category-list nav">
          <li
            v-for="(folder, idx) in folder_list"
            :key="folder"
            @click="click_category"
            :class="{ active: !idx }"
            data-bs-toggle="tab"
            :href="`#kt_tab_pane_${idx + 1}${idx + 1}${idx + 1}`"
          >
            {{ folder.name }}
            <span>{{ folder.files ? folder.files.length : 1 }}</span>
          </li>
        </ul>
      </div>
      <div class="search-result-item">
        <div class="tab-content" id="myTabContent">
          <div
            ...
            :key="folder"
            class="tab-pane fade"
            :class="{ show: !idx, active: !idx }"
            :id="`kt_tab_pane_${idx + 1}${idx + 1}${idx + 1}`"
            role="tabpanel"
          >
            <div class="search-result-item-button">
              <div class="btn-wrap d-flex">
                <button @click="check_file_all('check', folder)">
                  전체선택
                </button>
                <button @click="check_file_all('delete', folder)">
                  전체해제
                </button>
              </div>
            </div>
            ...
            <ul class="search-result-item-list d-flex align-items-center">
              ...
              >
                ...
                <label :for="`checkbox${item.id}`">{{
                  `${item.name}${item.version ? `_${item.version}` : ""}.${
                    item.extension
                  }`
                }}</label>
              </li>
    ...
    <div class="search-result-section2">
      <div class="title">
        관리파일 리스트 <span>{{ checked_file_sum }}건</span>
      </div>
      <ul class="search-result-section2-wrap">
        <li v-if="!checked_file_sum">선택한 관리파일이 없습니다.</li>
        <li
          v-for="item1 in checked_file_list_view"
          :key="item1"
          class="search-result-section2-folder"
        >
          <div
            v-if="item1.files.length"
          ...
          <ul v-if="item1.files.length" class="files d-flex">
            <li
              v-for="item2 in item1.files"
              :key="item2"
              class="d-flex align-items-center"
            >
              <img src="@/assets/img/group32.svg" alt="" />
              <span>{{
                `${item2.name}${item2.version ? `_${item2.version}` : ""}.${
                  item2.extension
                }`
              }}</span>
...
const init = async () => {
  ...
  folder_list.value = category_in_file(
    deepCopy(folders),
    files,
    "code",
    "folderCd"
...
let checked_file_list_view = computed(() =>
  category_in_file(
    deepCopy(origin_folder),
...
const check_file_all = (type, folder) => {
  ...
  if (type == "delete") {
    checked_file_list.value = checked_file_list.value.filter(
      (file) => file.folderCd != folder.code
    );
  }
};
